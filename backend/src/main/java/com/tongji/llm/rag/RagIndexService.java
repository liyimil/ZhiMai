...
public class RagIndexService {
    ...
    @Value("${rag.content-base-url:http://localhost:5173}")
    ...
    public int reindexSinglePost(long postId) {
        ...
        for (int i = 0; i < chunks.size(); i++) {
            ...
            meta.put("position", i);
            if (StringUtils.hasText(currentEtag)) {
                meta.put("contentEtag", currentEtag);
            }
            if (StringUtils.hasText(currentSha)) {
    ...
    private String fetchContent(String url) {
        try {
            ...
            if (!url.startsWith("http://") && !url.startsWith("https://")) {
                String base = contentBaseUrl.endsWith("/") ? contentBaseUrl : contentBaseUrl + "/";
                String path = url.startsWith("/") ? url.substring(1) : url;
                resolvedUrl = URI.create(base).resolve(path).toString();
            }
            byte[] content = http.getForObject(resolvedUrl, byte[].class);
